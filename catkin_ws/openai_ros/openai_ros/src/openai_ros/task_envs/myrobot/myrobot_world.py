...
from openai_ros.openai_ros_common import ROSLauncher, ROSLauncherWithParams
import os
...
class MyRobotWorldEnv(myrobot_env.MyRobotEnv):
    def __init__(self):
        ...
        # launch file params
        start_x = rospy.get_param('/myrobot/start_point_x')
        start_y = rospy.get_param('/myrobot/start_point_y')
        start_yaw = rospy.get_param('/myrobot/start_yaw')
        task_world_file = rospy.get_param('/myrobot/task_world_file')
        params = ['x:=' + str(start_x), 'y:=' + str(start_y), 'yaw:=' + str(start_yaw), 'world_file_name:=' + task_world_file]

        ROSLauncherWithParams(rospackage_name=task_ros_package, launch_file_name=task_launch_file, params=params, ros_ws_abspath=ros_ws_abspath)
