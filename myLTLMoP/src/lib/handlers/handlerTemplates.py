...
handler_type_mapping.update(OrderedDict([(v, k) for k, v in handler_type_mapping.items()]))
...
def getAllHandlerTypeName(short_name = True):
    ...
    return [h_type + ('Handler' if not short_name else '') \
            for h_type in handler_type_mapping.keys() if isinstance(h_type, str)]

def getAllHandlerTypeClass():
    """
    Return a list of handler type class
    """
    return [h_type for h_type in handler_type_mapping.keys() if not isinstance(h_type, str)]
