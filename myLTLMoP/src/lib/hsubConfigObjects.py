...
class HandlerMethodConfig(object):
    ...
    def updateParaFromDict(self, para_dict):
        ...
        for para_name, para_value in para_dict.items():
            # logging.warning(para_name)
            # logging.warning(para_value)
...
class HandlerConfig(object):
    ...
    def loadHandlerClass(self, handler_module_path):
        ...
        handler_class = inspect.getmembers(handler_module, lambda c: inspect.isclass(c) and c.__module__ == handler_module_path and ht.Handler in inspect.getmro(c))
        ...
        if len(handler_class) < 1:
            ...
            # logging.warning(handler_module)
            # logging.warning(handler_module_path)
            logging.warning(inspect.getmro(inspect.getmembers(handler_module, lambda c: inspect.isclass(c) and c.__module__ == handler_module_path)[0][1]))
            raise ImportError
    ...
    def loadHandlerMethod(self, handler_module_path, onlyLoadInit=False):
        ...
        handler_methods = inspect.getmembers(handler_class, lambda c: inspect.isfunction(c) or inspect.ismethod(c))
        # logging.warning(handler_methods)
    ...
    def fromData(self, robot_data, hsub):
        ...
        for key, val in robot_data.items():
            ...
            for handler_config_str in val:
                # Parse this line
                try:
                    # logging.warning(handler_config_str)
                ...
                # logging.warning(robot_type)
                # logging.warning(handler_type)
                # logging.warning(handler_name)
                ...
                try:
                    ...
                    # logging.warning(call_descriptors[0])
