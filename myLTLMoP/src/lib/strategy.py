...
class State(object):
    ...
    def satisfies(self, prop_assignments):
        ...
        return all((self.getPropValue(k) == v for k, v in prop_assignments.items()))
    ...
    def setPropValues(self, prop_assignments):
        ...
        for prop_name, prop_value in prop_assignments.items():
        ...
        with open(filename, 'w') as f_out:
            ...
            def pprint_assignment(name, val):
                if isinstance(val, bool):
                    ...
                    for rname, subregs in regionMapping.items():
            ...
            while states_to_process:
                ...
                state_label = "\\n".join((pprint_assignment(k, v)
                                          for k, v in this_state.getOutputs().items()))
                ...
                for next_state in self.findTransitionableStates({}, from_state=this_state):
                    trans_label = "\\n".join((pprint_assignment(k, v)
                                              for k, v in next_state.getInputs().items()))
...
def TestLoadAndDump(spec_filename):
    import project
    import pprint   # When running from the context of __name__ == "__main__", we need to
    ...
    region_map = proj.loadRegionMapping()

    ### Load the strategy
    region_domain = Domain("region", spec_map.regions, Domain.B0_IS_MSB)
    ...
    start_state = strat.searchForOneState({"region": initial_region})
    #pprint.pprint(strat.findTransitionableStates({}, from_state=start_state))

    ### Dump the strategy from this initial state
    strat.exportAsDotFile("strategy_test.dot", region_map, starting_states = [start_state])
