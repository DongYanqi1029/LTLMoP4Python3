    ...
    for title in sorted(data.keys()):
        ...
        for header in sorted(section.keys()):
            ...
            if header in comments:
                print(properCase(header) + ": # " + comments[header], file=f)
            else:
            ...
            for value in values:
                print(value, file=f)
            print(file=f)  # Put a blank line in between sections, just because it's prettier that way 
    f.close()
