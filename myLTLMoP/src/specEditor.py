...
class SpecEditorFrame(wx.Frame):
    ...
    def __init__(self, *args, **kwds):
        ...
        global MENU_IMPORT_REGION; MENU_IMPORT_REGION = wx.NewIdRef(count=1)
        global MENU_COMPILE; MENU_COMPILE = wx.NewIdRef(count=1)
        global MENU_COMPILECONFIG; MENU_COMPILECONFIG = wx.NewIdRef(count=1)
        global MENU_CONVEXIFY; MENU_CONVEXIFY = wx.NewIdRef(count=1)
        global MENU_FASTSLOW; MENU_FASTSLOW = wx.NewIdRef(count=1)
        global MENU_BITVECTOR; MENU_BITVECTOR = wx.NewIdRef(count=1)
        global MENU_PARSERMODE; MENU_PARSERMODE = wx.NewIdRef(count=1)
        global MENU_PARSERMODE_SLURP; MENU_PARSERMODE_SLURP = wx.NewIdRef(count=1)
        global MENU_PARSERMODE_STRUCTURED; MENU_PARSERMODE_STRUCTURED = wx.NewIdRef(count=1)
        global MENU_PARSERMODE_LTL; MENU_PARSERMODE_LTL = wx.NewIdRef(count=1)
        global MENU_SYNTHESIZER; MENU_SYNTHESIZER = wx.NewIdRef(count=1)
        global MENU_SYNTHESIZER_JTLV; MENU_SYNTHESIZER_JTLV = wx.NewIdRef(count=1)
        global MENU_SYNTHESIZER_SLUGS; MENU_SYNTHESIZER_SLUGS = wx.NewIdRef(count=1)
        global MENU_SYMBOLIC; MENU_SYMBOLIC = wx.NewIdRef(count=1)
        global MENU_SIMULATE; MENU_SIMULATE = wx.NewIdRef(count=1)
        global MENU_SIMCONFIG; MENU_SIMCONFIG = wx.NewIdRef(count=1)
        global MENU_ANALYZE; MENU_ANALYZE = wx.NewIdRef(count=1)
        global MENU_DOTTY; MENU_DOTTY = wx.NewIdRef(count=1)
        global MENU_MOPSY; MENU_MOPSY = wx.NewIdRef(count=1)
        ...
        wxglade_tmp_menu_sub.AppendSubMenu(wxglade_tmp_menu_sub_sub, "Parser mode", "")
        wxglade_tmp_menu_sub_sub = wx.Menu()
        ...
        wxglade_tmp_menu_sub.AppendSubMenu(wxglade_tmp_menu_sub_sub, "Synthesizer", "")
        ...
        wxglade_tmp_menu.AppendSubMenu(wxglade_tmp_menu_sub, "Compilation options", "")
        wxglade_tmp_menu.AppendSeparator()
    ...
    def doClose(self, event): # wxGlade: SpecEditorFrame.<event_handler>
        ...
        for n, p in self.subprocess.items():
            if p is not None:
    ...
    def highlightCores(self, guilty, compiler):
        if self.proj.compile_options["parser"] == "structured":
            ...
            if guilty is not None:
                ...
                for k,v in compiler.LTL2SpecLineNumber.items():
                    newCs = k.replace("\t","\n").split('\n')
            ...
            for ltl_frag in guilty:
                ...
                try:
                    ...
                    if "[]" not in canonical_ltl_frag:
                        ...
                        guilty_clean.extend([v for k,v in compiler.reversemapping.items() if "[]" not in v])
                    else:
